...
<<<<<<< HEAD
        ...
        self.insert = wx.Button(panel, label="新增", pos=(6*x0, y0+3*dy), size=(w, 25))
        self.insert.SetToolTipString("新增记录方法:\n1.在文本框中输入数据；\n2.单击此按钮以增加记录")
        
        self.delete = wx.Button(panel, label='删除', pos=(6*x0+w+dx, y0+3*dy), size=(w, 25))
        self.delete.SetToolTipString("删除记录方法:\n1.选择一行；\n2.单击此按钮以删除。")

        self.update = wx.Button(panel, label='修改', pos=(6*x0+2*(w+dx), y0+3*dy), size=(w, 25))
        self.update.SetToolTipString("修改记录方法:\n1.选择一行；\n2.在文本框中进行修改；\n3.单击此按钮以保存修改。")

        self.select = wx.Button(panel, label='查询', pos=(6*x0+3*(w+dx), y0+3*dy), size=(w, 25))
