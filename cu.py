...
def main(stdscr):
    ...
    stdscr.addstr("padhlines " + str(padhlines) +
                  " padwcols " + str(padwcols) + "; ")
    # both newpad and subpad are <class '_curses.curses window'>:
    mypadn = curses.newpad(padhlines, padwcols)
    mypads = stdscr.subpad(padhlines, padwcols, begin_y,
                           begin_x + padwcols + 4)
    ...
    mypadn.refresh(0, 0, begin_y, begin_x, begin_y +
                   hlines, begin_x + padwcols)
    mypads.refresh()
    mypads.touchwin()
    mypadn.touchwin()
